...
int getPosition(uint64_t matches){
    a = matches * -255; 
    b = a & LAST_BITS_ON;
    c = ~(b & (a>>1)) & b;
    d = c * -255;
    ...
    while(!(&*text_window.c > &text[text_len-1])) {
        ...
        if((bool)(query_matches = reduce_xnor(*text_window.i, query_matches, *char_ptr) & query_matches)) {
            if (&char_ptr[0] == last_char) {
                ...
                //get highest order match
                //invert query_matches
                match_count = (query_matches + query_matches/255) & 255;
                text_offset += getPosition(query_matches);
                text_window.c = &text[text_offset];
                if(text_window.i == query_window.i){
                    char_ptr += 8;
                    text_offset += 8;
                    text_window.c = &text[text_offset];
                }
